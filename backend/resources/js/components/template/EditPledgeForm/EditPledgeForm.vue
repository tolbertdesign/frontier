<template>
  <form
    novalidate
    role="form"
    class="w-full bg-white p-8 rounded-xl shadow-lg min-h-full overflow-y-scroll"
  ...
  >
    ...
    <div class="flex justify-between">
      ...
      <button
        class="text-black self-start cursor-pointer"
        @click.prevent="close"
      >
        <span class="icon">
          <FontAwesomeIcon
    ...
    <!-- State Select Input -->
    <div class="field mb-4">
      <label class="inline-block mb-1 pl-2 text-gray-700 text-sm">{{ lang.state_lbl }}</label>
      <div class="control">
    ...
    <div class="field mb-4">
      <label class="inline-block mb-1 pl-2 text-gray-700 text-sm">{{ lang.country_lbl }}</label>
      <div class="control">
        <div class="select w-full">
          <select
            id="country"
      ...
      <p
        ...
        class="help is-danger"
      >{{ lang.country_missing }}</p>
    </div>
    <!-- Sponsor Type Input -->
    <div class="field mb-4">
      <label class="inline-block mb-1 pl-2 text-gray-700 text-sm">{{ lang.sponsor_type_lbl }}</label>
      <div class="control">
        <div class="select w-full">
          <select
          ...
          >
            ...
            >{{ sponsor_type.sponsor_type }}</option>
          </select>
        </div>
      </div>
      <p
    ...
    <div class="mb-4 block">
      <label class="inline-block mb-1 pl-2 text-gray-700 text-sm">{{ lang.pledge_type_lbl }}</label>
      <div class="control">
        <div
          ref="pledge_type"
    ...
    <div class="px-8">
      <button
        :disabled="loading"
        type="submit"
        class="button is-secondary is-rounded is-fullwidth shadow-md mb-3 bg-secondary text-white font-bold border"
      >
        {{ lang.update_pledge }}
      </button>
      <button
        v-if="canDelete"
        :disabled="loading"
        class="button is-danger is-rounded is-fullwidth shadow-md mb-3 bg-red-700 text-white font-bold border"
      ...
      <button
        :disabled="loading"
        class="button is-outlined is-rounded is-fullwidth shadow-md mb-3 bg-white text-gray-600 font-bold border"
        @click.prevent="close"
      >
        {{ lang.cancel }}
      </button>
...
<script>
import { required, minValue, email, decimal } from 'vuelidate/lib/validators'
...
export default {
  name: 'EditPledgeForm',
  components: { TheMask, FontAwesomeIcon },
  mixins: [Blur],
  props: {
    canDelete: {
      type: Boolean,
  ...
  methods: {
    ...
    canEditAmount () {
      ...
      var hasPaymentInAScheduledState = false

      pledge.online_pending_payments.forEach(function (payment) {
        if (STATUS_PAYMENT_SCHEDULED.indexOf(payment.online_pending_payment_status_id) !== -1) {
        ...
        axios.put(url,
          ...
          .then(response => {
            if (response.status === 200) {
              ...
              this.loading = false
              this.$parent.$parent.$emit('pledge-updated')
              this.$emit('close')
    ...
    deletePledge () {
      ...
      axios.delete(url)
        .then(response => {
          this.$parent.$parent.$emit('pledge-deleted', this.pledge)
          this.$emit('close')
          this.submitStatus = 'OK'
        }).catch(err => {
          this.errors = [err.message]
