  ...
  >
    ...
    <main class="pt-24 lg:pt-40">
      <div class="mx-auto max-w-lg">
        ...
        <AddContactsButtons
          @addContactsToDisplay="addContactsToDisplay"
        />
        <ContactsTable
          ref="contactsTable"
...
export default {
  ...
  computed: {
    ...
    emailShareLink () {
      ...
      return window.location.protocol + '//' + window.location.hostname + '/v3/dash/' + specialUrl.short_key
    },
    participant () {
    ...
    getContacts () {
      ...
      sponsors.forEach(sponsor => {
        const isCurrentSponsor = currentSponsorEmails.indexOf(sponsor.email) !== -1
    ...
    getPotentialSponsors () {
      ...
      let potentialSponsors = this.participant.participant_info.potential_sponsors.filter(sponsor => sponsor.sender_user_id === this.$store.state.User.id)
      potentialSponsors = potentialSponsors
        .filter(sponsor => sponsor.participant_user_id === this.participant.participant_info.user_id)
  ...
  methods: {
    hasVideo () {
      return !!this.$store.getters.participants.filter(participant => participant.profile.video_url).length
    },
    ...
    getUnitExpectationLang () {
      if (this.isPPLTemplate()) {
        const unitExpectationLangArgs = {
          unitExpectationRange: this.participantProgram.unit_range_low + ' - ' + this.participantProgram.unit_range_high,
