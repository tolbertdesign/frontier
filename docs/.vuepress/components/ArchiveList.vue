...
<template>
  <div class="archive">
    <div v-for="year in getYears()" :key="year">
      ...
      <div v-for="month in getMonths(year)" :key="month">
        <div class="archive-month">
          {{ month | monthToLongName }}
        </div>
        <ul class="archive-list">
          <li v-for="(item, index) in postsByDate(year, month)" :key="index" class="archive-list__item">
            {{ new Date(item.frontmatter.date).getDate() }} - <router-link :to="item.path">
              {{ item.title }}
            </router-link>
          </li>
        </ul>
